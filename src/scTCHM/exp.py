import torch
import numpy as np
from .modules_SNV import scTCHM
from .dataset import ScDataManager, BulkDataManager
...
class scTCHMExperiment:
    def __init__(self, model_params, x_count, bulk_count, survival_time, cutting_off_0_1, x_batch_size, checkpoint, usePoisson_sc, 
        ...
        scTCHM_kwargs = {k: v for k, v in model_params.items() if k in {
            "x_dim", "z_dim", "h_dim",
    ...
    def elbo_loss(self, x, xnorm_mat, bulk_count, bulk_norm_mat, batch_onehot, bulk_idx):
        z, qz, x_hat, p_bulk, bulk_hat, theta_x, theta_bulk, beta_z, gamma_z_dict = self.scTCHM(x, batch_onehot, gene_name = None)
        if self.scTCHM.mode == 'sc':
            ...
            if not self.scTCHM.training:
                ...
                self.latest_c_index[self.scTCHM.mode] = c_uno
        return elbo_loss
    ...
    def train_epoch(self):
        ...
        for x, xnorm_mat, batch_onehot in self.x_data_manager.train_loader:
            ...
            self.scTCHM_optimizer.zero_grad()
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.scTCHM.parameters(), max_norm=5.0)
            self.scTCHM_optimizer.step()
            entry_num += x.shape[0]
    ...
    def evaluate(self, mode='test'):
        with torch.no_grad():
            ...
            loss = self.elbo_loss(x, xnorm_mat, self.bulk_count, self.bulk_norm_mat, batch_onehot, bulk_idx)
            entry_num = x.shape[0]
    ...
    def evaluate_train(self):
        with torch.no_grad():
            ...
            loss = self.elbo_loss(x, xnorm_mat, self.bulk_count, self.bulk_norm_mat, batch_onehot, idx)
            entry_num = x.shape[0]
            loss_val = loss / entry_num
    ...
    def train_total(self, epoch_num, patience):
        ...
        for epoch in range(epoch_num):
            ...
            if self.use_val_loss_mean == True:
                earlystopping(val_loss_mean, self.scTCHM)
            else:
                earlystopping(val_loss, self.scTCHM)
            if earlystopping.early_stop:
                print(f"Early Stopping! at {epoch} epoch, best score={earlystopping.best_score}")
                break
            if self.scTCHM.mode == 'beta_z':
                if epoch % 10 == 0:
    ...
    def initialize_optimizer(self, lr):
        self.scTCHM_optimizer = torch.optim.AdamW(self.scTCHM.parameters(), lr=lr)
    ...
    def get_beta_per_bin(self, x, batch_onehot):
        with torch.no_grad():
            ...
            z,_ = self.scTCHM.enc_z(torch.cat([x, boh], dim=-1))
            beta_z_all = self.scTCHM.dec_beta_z(z)
    ...
    def predict_lambda_table(self, bulk_idx):
        ...
        with torch.no_grad():
            ...
            (_, _, _,
            p_bulk_all, _, _, _,
            beta_z_all, gamma_z_dict) = self.scTCHM(x_all, boh_all)
